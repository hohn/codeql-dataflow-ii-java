...
public class AddUser {
  ...
  static int get_new_id() { return (int)(Math.random() * 100000); }
  ...
  static void writeNicknames(String name, LinkedList<String> nicknames) {
    try (Connection conn = connect()) {
      for (String nickname : nicknames) {
        String insertPart = "INSERT INTO nicknames ";
        String values = String.format(" VALUES ('%s', '%s')", name, nickname);
    ...
    try (Connection conn = connect()) {
      for (String nickname : nicknames.getNicknames()) {
        String query = String.format(
            "INSERT INTO nicknames VALUES ('%s', '%s')", name, nickname);
        conn.createStatement().executeUpdate(query);
        System.err.printf("Sent: %s", query);
      }
    } catch (SQLException e) {
